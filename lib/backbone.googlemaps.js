/*!
 * Backbone.GoogleMaps
 * A Backbone JS layer for the GoogleMaps API with MarkerClusterer integration
...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define(['backbone', 'underscore', 'markerclusterer', 'jquery'], factory);
  }
  else {
    // Browser globals
    factory(root.Backbone, root._, root.MarkerClusterer, root.jQuery || root.Zepto || root.ender);
  }
}(this, function(Backbone, _, MarkerClusterer, $) {
  ...
  GoogleMaps.MapView = Backbone.View.extend({
    ...
    _addGoogleMapsListener: function(topic, handler, opt_context) {
      ...
      this.listeners.push(google.maps.event.addListener(this.gOverlay, topic, handler));
    },
    ...
    close: function() {
      ...
      if (this.beforeClose) {
		...
		var gOverlay = this.gOverlay,
			listen = this.listener,
			comp = _.after(listen.length, function () {
				google.maps.event.clearInstanceListeners(gOverlay);
				if (gOverlay.setMap) {
					gOverlay.setMap(null);
				}
				gOverlay = null;					
			});
		_.each(this.listeners, function (m) {
  ...
  GoogleMaps.MarkerCollectionView = Backbone.View.extend({
    constructor: function(options) {
      ...
      // Add MarkerClusterer capability
      // - merge defaults
      var mcDefaults = _.extend({}, {
        active: true,
        opts: {
          averageCenter: true
        }
      }, this.options.markerclusterer);

      // - if set to active, instantiate
      if (mcDefaults.active) {
        mc = new MarkerClusterer(this.map, [], mcDefaults.opts);

      // - else if deactivated, create a no-op clone to avoid errors (and control structures everywhere)
      } else {
        var mc_proto  = _.extend({}, MarkerClusterer.prototype),
            fns       = _.functions(mc_proto),
            fn_arr    = [];
        _(fns.length).times(function(n){ fn_arr.push( function(){ return true; } ); });
        mc = _.object(fns, fn_arr);
      }

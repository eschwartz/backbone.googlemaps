...
Backbone.GoogleMaps = (function(Backbone, _, $){
	 ...
	 * GoogleMaps.Location
	 * --------------------
	 * Representing a lat/long location on a map
	*/
	GoogleMaps.Location = Backbone.Model.extend({
		constructor: function() {
			_.bindAll(this, 'select', 'deselect', 'toggleSelect', 'getLatLong');
	...
	*/
	GoogleMaps.MapView = Backbone.View.extend({
		...
		constructor: function() {
			...
			if(!this.options.map && !this.map) throw new Error("A map must be specified for InfoWindow view.");
	...
	*/
	GoogleMaps.InfoWindow = GoogleMaps.MapView.extend({
		...
		render: function() {
			...
			// Create InfoWindow
			this.gOverlay = new google.maps.InfoWindow({
				content: this.$el[0]
			});
	...
	GoogleMaps.MarkerView = GoogleMaps.MapView.extend({
		...
		constructor: function() {
			...
			this.gOverlay = new google.maps.Marker(_.extend({
				position: this.model.getLatLong(),
	...
	*/
	GoogleMaps.MarkerCollectionView = Backbone.View.extend({
		...
		constructor: function() {
			...
			// Ensure map property
			if(!this.options.map) throw new Error("A map must be specified on MarkerCollectionView instantiation");

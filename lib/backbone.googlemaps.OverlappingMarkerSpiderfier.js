...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define(['backbone', 'underscore', 'oms.min', 'backbone.picky', 'jquery'], factory);
  }
  else {
    // Browser globals
    factory(root.Backbone, root._, root.OverlappingMarkerSpiderfier, root.backbone.picky, root.jQuery || root.Zepto || root.ender);
  }
}(this, function(Backbone, _, OverlappingMarkerSpiderfier, Picky, $) {
  ...
  GoogleMaps.Location = Backbone.Model.extend({
    constructor: function() {
      _.bindAll(this, 'getLatLng', 'getLatlng');
  ...
  GoogleMaps.MarkerView = GoogleMaps.MapView.extend({
    constructor: function() {
      ...
      this.listenTo(this.model, "selected", this.openDetail, this);
	  this.listenTo(this.model, "deselected", this.closeDetail, this);
      this.listenTo(this.model, "change:lat change:lng", this.refreshOverlay, this);
    ...
    close: function() {
      ...
      // remove from spiderfier
      oms.removeMarker(this.gOverlay);
  ...
  GoogleMaps.MarkerCollectionView = Backbone.View.extend({
    constructor: function(options) {
	  ...
	  // Add OverlappingMarkerSpiderfier capability
	  // - merge defaults
      var omsDefaults = _.extend({}, {
        active: true,
        opts: {
          averageCenter: true
        }
      }, this.options.overlappingmarkerspiderfier);

      // - if set to active, instantiate
      if (omsDefaults.active) {
        oms = new OverlappingMarkerSpiderfier(this.map, [], omsDefaults.opts);

      // - else if deactivated, create a no-op clone to avoid errors (and control structures everywhere)
      } else {
        var mc_proto  = _.extend({}, OverlappingMarkerSpiderfier.prototype),
            fns       = _.functions(mc_proto),
            fn_arr    = [];
			console.log(fns);
        _(fns.length).times(function(n){ fn_arr.push( function(){ return true; } ); });
        oms = _.object(fns, fn_arr);
      }	  

      // Bind to collection
      this.listenTo(this.collection, "reset", this.refresh, this);
      this.listenTo(this.collection, "add", this.addChild, this);
      this.listenTo(this.collection, "remove", this.closeChild, this);	  
    },
